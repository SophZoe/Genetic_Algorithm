...
class Agent:
            ...
            if self.energy > ENERGYCOSTS_MOVEMENT:
                if self.sick is True:
                    ...
                    if new_position:
                        ...
                        # Zufällige Bewegung: -1 oder 1, multipliziert mit der Condition
                        dx = random.choice([-1, 1]) * self.genetic['Condition']
                        dy = random.choice([-1, 1]) * self.genetic['Condition']
    ...
    def try_to_reproduce(self, board):
        # Finde potenzielle Partner basierend auf der Position oder anderen Kriterien
        potential_partners = [agent for agent in board.agents_list if agent != self and self.can_reproduce_with(agent)]
        for partner in potential_partners:
            self.reproduce(partner, board)

    def can_reproduce_with(self, partner):
        # Prüft, ob die Fortpflanzungsbedingungen erfüllt sind
        return self.energy > ENERGYCOSTS_REPRODUCTION and self.position == partner.position

    def reproduce(self, partner, board):
        global agents_counter
        if random.random() < self.calculate_success_rate(partner):
            agents_counter += 1
            child = Agent(agents_counter)
            child.genedistribution_through_inheritance(self, partner)
            self.energy -= ENERGYCOSTS_REPRODUCTION
            self.reproduction_counter += 1
            partner.reproduction_counter += 1
            board.add_agent(child)

    def calculate_success_rate(self, partner):
        # Berechnet die Erfolgsrate der Fortpflanzung
        return 1 if self.genetic["Tribe"] == partner.genetic["Tribe"] else 0.3

    def genedistribution_through_inheritance(self, parent1, parent2):
        for gen in GENPOOL["Genes"]:
            if gen == "Tribe":
                ...
                gen_value = (mutation_rate * parent1.genetic[gen] + (1 - mutation_rate) * parent2.genetic[gen])
...
class Game:
    ...
    def save_data(self):
        ...
        with open(filename, 'w', newline='') as file:
            ...
            writer.writerow(['Number', ' Tribe', ' Condition', ' Visibility Range', 'Resistance' ' Metabolism', ' Covered Distance', ' Reproduction Counter', ' Consume Counter', ' Sickness Counter' ,' Parent A', ' Parent B', ' Position'])
            for agent in self.board.agents_list:
                writer.writerow([
                    agent.number, 
                    agent.genetic['Tribe'], 
                    agent.genetic['Condition'], 
                    agent.genetic['Visibilityrange'], 
                    agent.genetic['Resistance'], 
